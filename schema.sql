...
create index if not exists swipes_target_mode_action_idx on public.swipes (target_user_id, mode, action);
alter table public.swipes enable row level security;
drop trigger if exists swipes_set_updated_at on public.swipes;
create trigger swipes_set_updated_at before update on public.swipes for each row execute function public.handle_updated_at();
drop policy if exists "Swipers can view their own swipes" on public.swipes;
create policy "Swipers can view their own swipes" on public.swipes for select to authenticated using (auth.uid() = swiper_user_id);
drop policy if exists "Swipers can create their own swipes" on public.swipes;
create policy "Swipers can create their own swipes" on public.swipes for insert to authenticated with check (auth.uid() = swiper_user_id);
drop policy if exists "Swipers can update their own swipes" on public.swipes;
create policy "Swipers can update their own swipes" on public.swipes for update to authenticated using (auth.uid() = swiper_user_id) with check (auth.uid() = swiper_user_id);
drop policy if exists "Swipers can delete their own swipes" on public.swipes;
create policy "Swipers can delete their own swipes" on public.swipes for delete to authenticated using (auth.uid() = swiper_user_id);
...
create policy "Participants can view their matches" on public.matches for select to authenticated using (auth.uid() = user_a_id or auth.uid() = user_b_id);
